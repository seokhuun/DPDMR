...
class AGEM:
            ...
            for x, y in task_loader:
                ...
                if self.exemplar_sets:
                    ...
                    for exemplars, exemplar_labels in zip(self.exemplar_sets, self.exemplar_labels):
                        ...
                        # 모델 매개변수의 평균 경사를 계산
                        avg_gradients.append([p.grad.clone() for p in self.model.parameters()])

                    avg_gradients = [torch.stack(grads).mean(dim=0) for grads in zip(*avg_gradients)]

                    # A-GEM 경사 제한 적용
                    for p, avg_grad in zip(self.model.parameters(), avg_gradients):
                        if p.grad is not None:
                            dot_product = (p.grad * avg_grad).sum()
                            if dot_product < 0:
                                proj_coeff = dot_product / (avg_grad.norm() ** 2 + 1e-9)
                                p.grad -= proj_coeff * avg_grad
    ...
    def train_and_evaluate(self, tasks):
        for class_id, task in enumerate(tasks):
            print(f"\nTraining Class {class_id} with A-GEM:")
...
agem = AGEM(model, memory_size=400, lr=0.005, weight_decay=0.0001, gamma=0.2)  # A-GEM 파라미터 설정

# 클래스별 학습 및 중간 평가
agem.train_and_evaluate(tasks)
